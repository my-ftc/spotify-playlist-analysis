...
const UserPlaylistList: React.FC<UserPlaylistListProps> = ({ userPlaylists }) => {
    ...
    const paginatedPlaylists = userPlaylists.slice(startIndex, startIndex + itemsPerPage);

    return (
        <div className='mt-5 3xs:w-full md:w-1/2'>
            <div className="bg-white p-4 shadow-lg rounded-lg 3xs:text-sm xs:text-base">
                <h2 className="font-extrabold text-black xs:mt-3 xs:text-lg">Other playlists from user</h2>
                <p className='3xs:my-2.5 xs:my-4 text-[#515268]'>Explore more playlists from this user.</p>
            ...
            {totalPages > 1 && (
                <div className="flex justify-end mt-2 3xs:text-xs xs:text-base">
                    <Pagination
